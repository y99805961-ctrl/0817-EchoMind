...
test("Visual QA at 1440x900", async ({ page }) => {
  ...
  await page.screenshot({ path: resolve(screenshotDir, "nexusops-workspace-empty.png"), fullPage: true });
  ...
  await page.screenshot({ path: resolve(screenshotDir, "nexusops-workspace-result.png"), fullPage: true });

  await page.goto("/knowledge");
  await expect(page.getByTestId("knowledge-page")).toBeVisible();
  await page.screenshot({ path: resolve(screenshotDir, "nexusops-knowledge.png"), fullPage: true });

  await page.goto("/monitor");
  await expect(page.getByTestId("monitor-page")).toBeVisible();
  await page.screenshot({ path: resolve(screenshotDir, "nexusops-monitor.png"), fullPage: true });
});
